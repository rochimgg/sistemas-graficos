...
/*
    ...
    1) Modificar a función "generarSuperficie" para que tenga en cuenta los parametros filas y columnas al llenar el indexBuffer
       Con esta modificación deberían poder generarse planos de N filas por M columnas. ok

    2) Modificar la funcion "dibujarMalla" para que use la primitiva "triangle_strip". ok
...
function Esfera(radio){
    ...
    this.getPosicion=function(u,v){
        var x=radio*Math.cos(Math.PI*(u-.5))*Math.cos(2*Math.PI*v);
        var y=radio*Math.cos(Math.PI*(u-.5))*Math.sin(2*Math.PI*v);
        var z=radio*Math.sin(Math.PI*(u-.5));
        return [x,y,z];
    }
