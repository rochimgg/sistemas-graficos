        ...
        void main(void) {
           ...
           vec4 textureColor = texture2D(uSampler, vec2(uv.s, uv.t));         
            ...
            float x = uv.x-0.5;
            float y = uv.y-0.5;
            if(textureColor.rgb.b > textureColor.rgb.r){
                position=position*0.9;
                position+=normal*(1.0+0.5*sin(uv.x*18.0*PI+time*20.0))*0.03;
                position+=normal*(1.0+0.5*sin(uv.y*18.0*PI+time*30.0))*0.03;
